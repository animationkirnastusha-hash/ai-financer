...
export function StoreCardGrid({ cards, hasPremium, hasBusiness, onCardClick }: Props) {
  ...
  return (
    <section className="store-card-grid">
      {cards.map((card) => {
        const active = card.comingSoon ? false : card.tone === 'premium' ? hasPremium : card.tone === 'business' ? hasBusiness : false;
        return (
          <article key={card.title} className={`store-card store-card--${card.tone}`}>
            <button type="button" className="store-card__button" onClick={() => onCardClick(card)} aria-label={t(card.title)}>
              <div className="store-card__head">
                ...
                {card.comingSoon ? <span>{t('store.status.soon')}</span> : active ? <span>{t('store.status.active')}</span> : null}
              </div>
              <h2>{t(card.title)}</h2>
              <p>{t(card.caption)}</p>
              <ul>
              ...
              <span className="store-card__action">{card.comingSoon ? t('store.action.businessSoon') : t(card.action)}</span>
            </button>
          </article>
        );
      })}
