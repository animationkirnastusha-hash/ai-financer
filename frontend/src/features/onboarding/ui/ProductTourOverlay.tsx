...
function getCardStyle(targetRect: TargetRect | null): CSSProperties {
  if (!targetRect) {
    return {
      left: '50%',
      bottom: 22,
  ...
  const cardWidth = Math.min(342, Math.max(284, window.innerWidth - 32));
  const left = clamp(targetRect.left + targetRect.width / 2 - cardWidth / 2, 16, window.innerWidth - cardWidth - 16);
  const placeAbove = targetRect.top > window.innerHeight * 0.54;
  const top = placeAbove
    ? clamp(targetRect.top - 188, 16, window.innerHeight - 204)
    : clamp(targetRect.bottom + 16, 16, window.innerHeight - 204);
...
export function ProductTourOverlay() {
  ...
  useEffect(() => {
    ...
    timer = window.setTimeout(updateTarget, 190);
    updateTarget();
  ...
  return (
    <div className="product-tour" role="dialog" aria-modal="true" aria-label={t('onboarding.tour.aria')} data-no-swipe="true">
      <div className="product-tour__veil" aria-hidden="true" />
      {targetRect ? <div className="product-tour__spotlight" aria-hidden="true" style={spotlightStyle} /> : null}

      <section className="product-tour__card" style={getCardStyle(targetRect)}>
        ...
        <div className="product-tour__actions">
          ...
          <div>
            ...
            <button type="button" className="app-primary-button app-primary-button--compact" onClick={next}>
