  ...
  if (compact) {
    return (
      <div className={shellClass}>
        ...
        <div className={stats.balance >= 0 ? 'mt-2 text-lg font-semibold tracking-[-0.03em] text-emerald-300' : 'mt-2 text-lg font-semibold tracking-[-0.03em] text-white'}>
          {formatMoney(stats.balance, 'RUB', { sign: 'auto' })}
        </div>
        {stats.topCategories.length > 0 ? (
          <div className="mt-3 space-y-2">
            {stats.topCategories.slice(0, 2).map((category) => (
              <div key={category.name} className="flex items-center justify-between gap-2 rounded-2xl bg-black/18 px-3 py-2">
