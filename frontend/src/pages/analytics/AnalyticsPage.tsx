    ...
    <div className="h-full overflow-y-auto px-4 pb-28 pt-[calc(env(safe-area-inset-top)+70px)] text-white">
      <div className="mx-auto max-w-[620px] space-y-4">
        <header className="rounded-[34px] border border-white/10 bg-white/[0.045] p-5">
          <div className="text-[11px] uppercase tracking-[0.2em] text-emerald-200/60">Аналитика</div>
          <h1 className="mt-3 text-3xl font-semibold tracking-[-0.04em]">Расходы и изменения</h1>
          <p className="mt-2 text-sm leading-6 text-white/55">Короткие ответы по тратам, категориям и динамике.</p>
        </header>
        ...
        <section className="rounded-[30px] border border-white/10 bg-white/[0.04] p-5">
          ...
          <div className="mt-4 space-y-3">
            {data.top.length === 0 ? <div className="text-sm text-white/45">Добавь первые расходы — здесь появится структура.</div> : data.top.map(([name, value]) => (
              <div key={name}>
        ...
        <section className="rounded-[30px] border border-amber-200/12 bg-amber-200/[0.06] p-5">
          ...
          <p className="mt-2 text-sm leading-6 text-white/52">Прогнозы, повторяющиеся траты и месячные отчёты будут доступны в расширенном режиме.</p>
          <button onClick={() => navigateTo('premium')} className="mt-4 rounded-full border border-white/10 bg-white/[0.05] px-4 py-2 text-sm text-white/70">Посмотреть Премиум</button>
        </section>
      </div>
    </div>
  );
