...
function groupTransactions(
  ...
  const grouped = transactions.reduce<Record<string, MoneyGroup>>(
    (acc, item) => {
      const category = item.category;
      const section = item.section ?? category?.section ?? null;
      const name =
        category?.name || section?.name || item.title || fallbackName;
...
function buildDailyPoints(
  ...
  transactions.forEach((item) => {
    ...
    if (item.type === "income")
      points[index].income += Number(item.amount || 0);
    if (item.type === "expense")
      points[index].expense += Number(item.amount || 0);
  });
...
function formatCompactMoney(value: number) {
  ...
  if (abs >= 1_000_000)
    return `${value < 0 ? "-" : ""}₽${Math.round(abs / 100_000) / 10}м`;
  if (abs >= 1_000)
    return `${value < 0 ? "-" : ""}₽${Math.round(abs / 100) / 10}к`;
  ...
  return (
    <div className="analytics-v2-donut" aria-label={label} role="img">
      <svg viewBox="0 0 120 120" aria-hidden="true">
        <circle
          className="analytics-v2-donut__track"
          cx="60"
          cy="60"
          r={radius}
          strokeWidth={stroke}
...
function CategoryList({
  ...
  if (groups.length === 0) {
    return (
      <div className="analytics-v2-empty">{t("analytics.v2.empty.groups")}</div>
    );
  }
  ...
  return (
    <div className="analytics-v2-category-list">
      {groups.map((group) => {
        const percent =
          total > 0 ? Math.round((group.amount / total) * 100) : 0;
        return (
          <button
            key={group.key}
            type="button"
            className="analytics-v2-category-row"
            onClick={() => onOpen(group)}
          >
            <span
              className="analytics-v2-category-row__icon"
...
function LineChart({
  ...
  const values = points.map((point) =>
    mode === "balance" ? point.balance : point[mode],
  );
  ...
  return (
    <div className={`analytics-v2-line analytics-v2-line--${mode}`}>
      <div className="analytics-v2-line__summary">
        ...
        <small>
          {t("analytics.v2.chart.peak")}: {formatCompactMoney(peak)}
        </small>
      </div>
      <svg
        ...
        role="img"
        aria-label={t("analytics.v2.section.dynamic")}
      >
        <line
          className="analytics-v2-line__grid"
          x1={padX}
          x2={width - padX}
          y1={padY}
          y2={padY}
        ...
        />
        <line
          className="analytics-v2-line__zero"
          x1={padX}
          x2={width - padX}
          y1={zeroY}
          y2={zeroY}
        />
        {area ? (
          <polygon className="analytics-v2-line__area" points={area} />
        ) : null}
        {line ? (
          <polyline className="analytics-v2-line__path" points={line} />
        ) : null}
        {coordinates.map((item, index) =>
          item.value !== 0 || index === coordinates.length - 1 ? (
            <circle
              key={item.point.key}
              className="analytics-v2-line__dot"
              cx={item.x}
              cy={item.y}
              r="3.2"
            />
...
function CalendarRangePicker({
  ...
  const weekdays = t("analytics.v2.calendar.weekdays").split("|");
  const days = buildCalendarDays(visibleMonth);
  const startDate = draftStart
    ? fromDateInputValue(draftStart, visibleMonth)
    : null;
  const endDate = draftEnd ? fromDateInputValue(draftEnd, visibleMonth) : null;
  ...
  return (
    <div className="analytics-v2-calendar-panel">
      <div className="analytics-v2-calendar-panel__head">
        <button
          type="button"
          aria-label={t("analytics.v2.calendar.prev")}
        ...
        <button
          type="button"
          aria-label={t("analytics.v2.calendar.next")}
          onClick={() => onVisibleMonthChange(addMonths(visibleMonth, 1))}
        >
          ›
        </button>
      ...
      <div className="analytics-v2-calendar-panel__hint">
        {canApply
      ...
      <div className="analytics-v2-calendar-weekdays" aria-hidden="true">
        {weekdays.map((day) => (
          <span key={day}>{day}</span>
        ))}
      </div>

      <div className="analytics-v2-calendar-grid">
        {days.map((date) => {
      ...
      <div className="analytics-v2-calendar-actions">
        <button
          type="button"
          className="analytics-v2-calendar-actions__secondary"
          onClick={onCancel}
        >
          {t("common.cancel")}
        </button>
        <button
          type="button"
          className="analytics-v2-calendar-actions__primary"
          disabled={!canApply}
          onClick={onApply}
        >
          ✓
...
export default function AnalyticsPage() {
  ...
  const [overviewStructureMode, setOverviewStructureMode] =
    useState<MoneyMode>("expense");
  const [overviewDynamicMode, setOverviewDynamicMode] =
    useState<MoneyMode>("expense");
  ...
  const [calendarDraftStart, setCalendarDraftStart] = useState(customStart);
  const [calendarDraftEnd, setCalendarDraftEnd] = useState(customEnd);
  const [calendarMonth, setCalendarMonth] = useState(() =>
    fromDateInputValue(customStart, new Date()),
  );
  ...
  const analytics = useMemo(() => {
    ...
    const current = transactions.filter((item) =>
      isInsideRange(item.date, range),
    );
    const previous = transactions.filter((item) =>
      isInsideRange(item.date, previousRange),
    );
    ...
    const expenseGroups = groupTransactions(
      expenses,
      t("analytics.uncategorized"),
    );
    const incomeGroups = groupTransactions(
      income,
      t("analytics.income.default"),
    );
    ...
    const averageExpense =
      dailyPoints.length > 0 ? totalExpense / dailyPoints.length : 0;
    const averageIncome =
      dailyPoints.length > 0 ? totalIncome / dailyPoints.length : 0;
  ...
  const finaInsight = useMemo(() => {
    ...
    if (activeTab === "expense") {
      if (analytics.topExpense)
        return t("analytics.v2.fina.expenseFocus", {
          category: rt(analytics.topExpense.name),
          amount: formatMoney(analytics.topExpense.amount, "RUB"),
        });
      return t("analytics.v2.fina.expenseCalm");
    }

    if (activeTab === "income") {
      if (analytics.topIncome)
        return t("analytics.v2.fina.incomeFocus", {
          category: rt(analytics.topIncome.name),
          amount: formatMoney(analytics.topIncome.amount, "RUB"),
        });
    ...
    if (activeTab === "balance") {
      if (analytics.balance < 0)
        return t("analytics.v2.fina.negative", {
          amount: formatMoney(Math.abs(analytics.balance), "RUB"),
        });
      return t("analytics.v2.fina.balanceFocus", {
        percent: Math.round(analytics.profitRatio),
      });
    }
    ...
    if (analytics.expenseRatio > 80)
      return t("analytics.v2.fina.warning", {
        percent: Math.round(analytics.expenseRatio),
      });
    if (analytics.topExpense)
      return t("analytics.v2.fina.topExpense", {
        category: rt(analytics.topExpense.name),
        amount: formatMoney(analytics.topExpense.amount, "RUB"),
      });
    ...
    activeTab,
    analytics.balance,
    analytics.current.length,
    analytics.expenseRatio,
    analytics.profitRatio,
  ...
  const openGroupJournal = (group: MoneyGroup, type: "income" | "expense") => {
    openJournal({
      ...
      type,
      query: group.name,
      tag: normalizeJournalTag(group.name),
    });
  };

  const openCalendar = () => {
    setPeriodPreset("custom");
    setCalendarDraftStart(customStart);
    setCalendarDraftEnd(customEnd);
  ...
  const selectCalendarDate = (value: string) => {
    if (!calendarDraftStart || calendarDraftEnd) {
      setCalendarDraftStart(value);
      setCalendarDraftEnd("");
      return;
    }

    if (value < calendarDraftStart) {
      setCalendarDraftEnd(calendarDraftStart);
      setCalendarDraftStart(value);
      return;
    }

    setCalendarDraftEnd(value);
  };

  const applyCalendarRange = () => {
    if (!calendarDraftStart || !calendarDraftEnd) return;
    setCustomStart(calendarDraftStart);
    setCustomEnd(calendarDraftEnd);
  ...
  const structureGroups =
    structureMode === "expense"
      ? analytics.expenseGroups
      : analytics.incomeGroups;
  ...
  const structureTitle =
    structureMode === "expense"
      ? t("analytics.v2.chart.expense")
      : t("analytics.v2.chart.income");
  const dynamicTitle =
    dynamicMode === "expense"
      ? t("analytics.v2.dynamic.expense")
      : t("analytics.v2.dynamic.income");
  ...
  return (
    <div className="app-page app-analytics-page analytics-v2-page text-white">
      <div className="app-page__inner analytics-v2-page__inner">
        ...
        <section
          className="analytics-v2-tabs"
          aria-label={t("analytics.v2.tabs.label")}
        >
          {tabs.map((tab) => (
            <button
              key={tab.value}
        ...
        <section
          className="analytics-v2-period app-card"
          ...
          {periodPreset === "custom" ? (
            <div className="analytics-v2-period__range">
              <button type="button" onClick={openCalendar}>
                ...
                <strong>{formatPeriodLabel(analytics.range)}</strong>
              </button>
            </div>
          ) : null}
          {periodPreset === "custom" && calendarOpen ? (
            <CalendarRangePicker
        ...
        {activeTab === "overview" ? (
          <>
            <section
              className="analytics-v2-kpi-grid analytics-v2-kpi-grid--overview"
              aria-label={t("analytics.v2.kpi.label")}
            >
              <KpiCard
                label={t("analytics.kpi.expense")}
                value={formatMoney(analytics.totalExpense, "RUB", {
                  sign: "minus",
                })}
              ...
              <KpiCard
                ...
                value={formatMoney(analytics.totalIncome, "RUB", {
                  sign: "plus",
                })}
              ...
              />
              <KpiCard
                label={t("analytics.v2.kpi.balance")}
                value={formatMoney(analytics.balance, "RUB", { sign: "auto" })}
            ...
            <section className="app-card analytics-v2-news">
              <div className="analytics-v2-section-head">
                ...
                <button
                  type="button"
                  className="analytics-v2-light-button"
                  onClick={() =>
                    openJournal({
                      period:
                        periodPreset === "day"
                ...
                >
                  {t("analytics.journal.action")}
                </button>
              </div>
              <div className="analytics-v2-news-grid">
                <NewsCard
                  label={t("analytics.v2.news.balance")}
                  value={formatMoney(analytics.balance, "RUB", {
                    sign: "auto",
                  })}
                  caption={t("analytics.v2.news.balanceCaption")}
                />
                <NewsCard
                  label={t("analytics.v2.news.topExpense")}
                  value={
                    analytics.topExpense ? rt(analytics.topExpense.name) : "—"
                  }
                  caption={
                    analytics.topExpense
                      ? formatMoney(analytics.topExpense.amount, "RUB")
                      : t("analytics.v2.empty.groups")
                  }
                />
                <NewsCard
                  label={t("analytics.v2.news.activity")}
                  value={String(analytics.current.length)}
                  caption={t("analytics.v2.balance.operations")}
                />
              </div>
            </section>
          </>
        ...
        {activeTab === "expense" ? (
          <section
            className="analytics-v2-focus-grid"
            aria-label={t("analytics.v2.tab.expense")}
          >
            <KpiCard
              label={t("analytics.kpi.expense")}
              value={formatMoney(analytics.totalExpense, "RUB", {
              ...
              change={analytics.expenseChange}
              tone="expense"
              caption={t("analytics.v2.expense.totalCaption")}
            />
            <KpiCard
              label={t("analytics.v2.expense.average")}
              value={formatMoney(analytics.averageExpense, "RUB")}
              change={0}
              tone="expense"
              caption={t("analytics.v2.expense.averageCaption")}
            />
          </section>
        ) : null}
        ...
        {activeTab === "income" ? (
          <section
            className="analytics-v2-focus-grid"
            aria-label={t("analytics.v2.tab.income")}
          >
            <KpiCard
              label={t("analytics.kpi.income")}
              value={formatMoney(analytics.totalIncome, "RUB", {
              ...
              change={analytics.incomeChange}
              tone="income"
              caption={t("analytics.v2.income.totalCaption")}
            />
            <KpiCard
              label={t("analytics.v2.income.average")}
              value={formatMoney(analytics.averageIncome, "RUB")}
              change={0}
              tone="income"
              caption={t("analytics.v2.income.averageCaption")}
            />
          </section>
        ) : null}
        ...
        {activeTab === "balance" ? (
          <section
            className="analytics-v2-focus-grid analytics-v2-focus-grid--balance"
            aria-label={t("analytics.v2.tab.balance")}
          >
            <KpiCard
              label={t("analytics.v2.kpi.balance")}
              value={formatMoney(analytics.balance, "RUB", { sign: "auto" })}
              change={analytics.balanceChange}
              tone="balance"
              caption={t("analytics.v2.balance.netCaption")}
            />
            <KpiCard
              label={t("analytics.v2.balance.expenseRatio")}
              value={`${Math.round(analytics.expenseRatio)}%`}
              change={0}
              tone="balance"
              caption={t("analytics.v2.balance.expenseRatioCaption")}
            />
            <KpiCard
              label={t("analytics.v2.balance.profitRatio")}
              value={`${Math.round(analytics.profitRatio)}%`}
              change={0}
              tone="balance"
              caption={t("analytics.v2.balance.profitRatioCaption")}
            />
          </section>
        ) : null}
              ...
              {activeTab === "overview" ? (
                <div className="analytics-v2-switch">
                  <button
                    type="button"
                    className={
                      overviewStructureMode === "expense" ? "is-active" : ""
                    }
                    onClick={() => setOverviewStructureMode("expense")}
                  >
                    {t("analytics.kpi.expense")}
                  </button>
                  <button
                    type="button"
                    className={
                      overviewStructureMode === "income" ? "is-active" : ""
                    }
                    onClick={() => setOverviewStructureMode("income")}
                  >
                    {t("analytics.kpi.income")}
                  </button>
                </div>
              ) : null}
            </div>
        ...
        <section className="analytics-v2-fina app-card">
          <div className="analytics-v2-fina__avatar" aria-hidden="true">
            <span />
            <span />
          </div>
          <div>
              ...
              {activeTab === "overview" ? (
                <div className="analytics-v2-switch">
                  <button
                    type="button"
                    className={
                      overviewDynamicMode === "expense" ? "is-active" : ""
                    }
                    onClick={() => setOverviewDynamicMode("expense")}
                  >
                    {t("analytics.kpi.expense")}
                  </button>
                  <button
                    type="button"
                    className={
                      overviewDynamicMode === "income" ? "is-active" : ""
                    }
                    onClick={() => setOverviewDynamicMode("income")}
                  >
                    {t("analytics.kpi.income")}
                  </button>
                </div>
          ...
          <section className="app-card analytics-v2-detail-card">
            <div className="analytics-v2-section-head">
              <div>
                <span>
                  {activeTab === "expense"
                    ? t("analytics.expenses.eyebrow")
                    : t("analytics.income.eyebrow")}
                </span>
                <h2>
                  {activeTab === "expense"
                    ? t("analytics.v2.detail.expense")
                    : t("analytics.v2.detail.income")}
                </h2>
              </div>
            </div>
            <CategoryList
          ...
          <button
            ...
            onClick={() =>
              openModal({
                ...
                initialCommand: t("analytics.ask.topExpense"),
                autoSubmitInitialCommand: true,
              })
            }
          >
            {t("analytics.v2.action.ask")}
          </button>
          <button
            type="button"
            onClick={() => openModal({ type: "report-export", mode: "base" })}
          >
            {t("analytics.report.action")}
          </button>
        </section>
      </div>
    </div>
