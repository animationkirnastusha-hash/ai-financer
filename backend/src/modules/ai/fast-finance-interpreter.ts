...
function normalizeAccountName(value: string) {
  return value
    .replace(/[«»"']/g, '')
    .replace(/\b(и|а|потом|затем|далее|после этого|туда|сюда|на него|на нее|на неё|положи|положить|закинь|закинуть|внеси|внести|пополнить|пополни|добавь|добавить|зачисли|зачислить)\b/gi, ' ')
    .replace(/\b(на сумму|с балансом|балансом|баланс|рублей|рубля|руб|долларов|доллара|доллар|баксов|бакса|бакс|евро)\b/gi, ' ')
...
function extractNamedAccount(input: string) {
  ...
  const named = input.match(/(?:назови|назвать)\s+(?:его|ее|её|счет|счёт|карту|кошелек|кошелёк)?\s+(.+?)(?:\s+(?:и|потом|затем|положи|закинь|внеси|пополни|добавь|на сумму|с балансом)\b|$)/i)?.[1];
...
function extractAccountNameFromCreate(input: string, currency: 'RUB' | 'USD' | 'EUR') {
  ...
  const explicit = input.match(/(?:счет|счёт|карту|кошелек|кошелёк)\s+[«"]?(.+?)(?:[»"]|\s+(?:и|потом|затем|положи|закинь|внеси|пополни|добавь|на сумму|с балансом|на|в)\b|$)/i)?.[1];
