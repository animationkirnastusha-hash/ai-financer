...
export class AIPlannerService {
  ...
  private systemPrompt() {
    return [
      ...
      'Use semantic understanding only. Do not rely on word-list shortcuts or regex-like extraction.',
      ...
      'For create_transaction, use title only for a short clean operation label. Do not copy the full user phrase into title. Put store/place/business name into merchant/place, item meanings into items, and extra searchable context into tags or description.',
  ...
  private buildPrompt(command: string, context: unknown) {
    return [
      ...
      'Plan by semantic meaning and tool contracts only. No regex-like or phrase-list financial extraction.',
      ...
      'For every transaction, provide category and section when the meaning is clear from the whole request; leave them absent only when genuinely unclear. Category and section must describe what the money was for, not merely where the user paid.',
      'Do not use a shop/place/merchant as the transaction title or category when the user describes what was bought. Put place/merchant into merchant/place, and keep title/category/section based on the purchase meaning.',
      'If one total amount contains several item meanings without item-level prices, create one transaction only. Do not split the amount. Put item meanings into items and keep one clean category/section for the dominant or mixed meaning.',
  ...
  private async runFocusedPlanner(command: string, context: unknown): Promise<Record<string, unknown>> {
    ...
    return this.provider.generateJson<Record<string, unknown>>({
      system: [
        ...
        'Do not use examples, regex patterns, or word-list shortcuts to extract financial data.',
