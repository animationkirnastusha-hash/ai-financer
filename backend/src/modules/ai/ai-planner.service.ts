...
export class AIPlannerService {
  ...
  private systemPrompt() {
    return [
      ...
      'For mixed purchases with one total amount and no item-level prices, create exactly one transaction. Put every named purchased object/service into items as a string array, keep merchant/place separately, and choose a broad honest category/section only when semantically clear. If the merchant/place is a gas station and the items are mixed, prefer category “Покупки на АЗС” and section “Авто”; never classify that as “Продукты” just because one item is food or drink.',
      ...
      'Transaction title must be short and clean. Do not copy the full spoken command, do not put amount/account into title, and do not use “merchant: items”, “merchant, items” or a list of purchased items as title. Use merchant/place/items/tags/description fields for those details.',
  ...
  private buildPrompt(command: string, context: unknown) {
    return [
      ...
      'For create_transaction: kind is income or expense; amount is one positive number; account is natural account name; title is a short label; category/section are financial meaning; merchant/place/items/tags/description carry context. items must be an array when named goods/services are present. For a mixed purchase under one total, do not make title equal to the item list or the merchant plus item list.',
