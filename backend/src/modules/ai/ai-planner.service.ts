...
export class AIPlannerService {
  ...
  private systemPrompt() {
    return [
      ...
      'For mixed purchases with one total amount and no item-level prices, create exactly one transaction. Put every named purchased object/service into items as a string array, keep merchant/place separately, and choose one broad honest category only when semantically clear.',
  ...
  private buildPrompt(command: string, context: unknown) {
    return [
      ...
      'Use CTX entity names when the user clearly refers to an existing account, category, goal, loan or transaction. Different endings and colloquial forms may refer to the same entity.',
      'Financial mutations must use the matching tool family: transactions, transfers, accounts, goals, obligations, categories, settings, onboarding, analytics.',
      'For create_transaction: kind is income or expense; amount is one positive number; account is natural account name; title is a short label; category is the broad financial meaning; merchant/place/items/tags/description carry context. items must be an array when named goods/services are present. Do not use sections.',
      'For income, category should describe income source or broad income group, not the account receiving money.',
      'For expense, category should describe purpose. If purpose has several unrelated items under one total, do not split the total; use one transaction with structured items/tags and a broad category. If no purpose category is confident, leave category empty instead of using generic Expense/Расход.',
      ...
      'Merchant/place must not replace category unless the user explicitly asks to create or rename that category.',
      'If no existing category fits and the meaning is still clear, output a concise broad category name; backend may create it. If meaning is unclear, leave category empty.',
  ...
  private async runFocusedPlanner(command: string, context: unknown): Promise<Record<string, unknown>> {
    ...
    return this.provider.generateJson<Record<string, unknown>>({
      ...
      prompt: [
        ...
        'Pass user-provided natural names through as tool input values. For amount fields, return unambiguous compact amounts as plain numeric values; if unsure, leave amount missing. For compact notes like account + item price + item price, preserve the account, sum clear item prices, and keep item words in items.',
        ...
        'Keep user-provided account/category labels natural. Context is not a picklist and must not override the user message.',
        'If no category fits confidently, leave category empty instead of misclassifying. A place is not a category.',
    ...
    try {
      const raw = await this.provider.generateJson<Record<string, unknown>>({
        system: [
          ...
          'For create_transaction, category describes financial purpose; merchant/place describe where it happened; items/tags/description preserve concrete purchase context.',
          ...
          'If draft uses a generic category such as Expense/Income/Расход/Доход only to avoid an empty field, remove that field. Validator can ask or use fallback safely.',
