...
export class AIPlannerService {
  ...
  private buildPrompt(command: string, context: unknown) {
    return [
      ...
      'Do not parse the command with rules. Plan by semantic meaning and tool contracts only. No regex-like financial extraction and no manual financial interpretation outside the tool contract.',
  ...
  private async runFocusedPlanner(command: string, context: unknown): Promise<Record<string, unknown>> {
    ...
    return this.provider.generateJson<Record<string, unknown>>({
      ...
      prompt: [
        ...
        'RULES:',
        'Use only listed tools.',
        'No hard-coded phrase handling. No manual financial interpretation outside the tool contract.',
