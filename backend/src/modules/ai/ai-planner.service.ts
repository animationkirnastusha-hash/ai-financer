  ...
  private async runFocusedPlanner(command: string, context: unknown): Promise<Record<string, unknown>> {
    ...
    return this.provider.generateJson<Record<string, unknown>>({
      system: [
        ...
        'For transactions, separate category/section from merchant/place/items/tags. Preserve named goods/services in items. Preserve the user-provided natural account reference unless a concrete CTX entity was clearly selected. Never copy the raw user sentence as title.',
