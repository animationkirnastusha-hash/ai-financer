...
export class AIDialogRouterService {
  ...
  private systemPrompt() {
    return [
      ...
      "Use tools when the user wants to change app data, open/show app data, ask a data-backed finance question, or record a completed financial fact.",
  ...
  private buildPrompt(command: string, context: unknown, tier: AIUserTier) {
    return [
      ...
      "ROUTING_GUIDE:",
      "- financial_action: user wants to create/update/delete/record/pay/transfer/set something in the app, including a finished money movement that should affect balances. shouldUseTools=true.",
