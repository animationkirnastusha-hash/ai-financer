...
export class AIPreviewService {
  buildParsed(summary: string, actions: AIValidatedAction[]): AIParsedCommand {
    ...
    return {
      intent: "batch",
      summary: this.normalizeSummary(summary, enriched),
      actions: enriched,
    };
  }
  ...
  buildExecutedMessage(parsed: AIParsedCommand) {
    if (
      ...
      parsed.actions[0]?.tool === "create_account"
    ) {
  ...
  private normalizeSummary(summary: string, actions: AIValidatedAction[]) {
    ...
    if (
      !clean ||
      /^проверь действие/i.test(clean) ||
      /^проверь \d+/i.test(clean)
    ) {
  ...
  private describeAccountType(type: unknown) {
  ...
  private describeAction(action: AIValidatedAction) {
    ...
    if (tool === "create_transaction") {
      ...
      const name =
        this.clean(input.description || input.category) ||
        (input.kind === "income" ? "Доход" : "Расход");
    ...
    if (tool === "update_transaction") {
      const target =
        this.clean(input.transaction) || this.clean(input.target) || "операцию";
      const changes = [
        input.amount !== undefined
          ? `сумма ${this.formatAmount(input.amount, input.currency)}`
          : "",
        input.description !== undefined
          ? `описание: ${this.clean(input.description) || "пусто"}`
          : "",
        input.account !== undefined ? `счёт: ${this.clean(input.account)}` : "",
        input.category !== undefined
          ? `категория: ${this.clean(input.category)}`
          : "",
        input.section !== undefined
          ? `раздел: ${this.clean(input.section)}`
          : "",
        input.kind !== undefined
          ? `тип: ${input.kind === "income" ? "доход" : input.kind === "expense" ? "расход" : "перевод"}`
          : "",
      ]
        .filter(Boolean)
        .join(", ");
    ...
    if (tool === "create_account") {
      if (input.__skipCreate)
        return `счёт "${this.clean(input.name) || "без названия"}" уже существует`;
      const initialBalance = Number(input.initialBalance ?? 0);
      const typeLabel = this.describeAccountType(input.type);
      return `создать ${typeLabel} "${this.clean(input.name) || "без названия"}"${initialBalance > 0 ? ` с балансом ${this.formatAmount(initialBalance, input.currency)}` : ""}`;
    }
    ...
    if (tool === "update_account") {
      ...
      return name
        ? `переименовать счёт ${account} в ${name}`
        : `изменить счёт ${account}`;
    }
    ...
    if (tool === "update_spending_limit") {
      const amount =
        input.amount !== undefined
          ? ` на ${this.formatAmount(input.amount, input.currency)}`
          : "";
    ...
    if (tool === "show_companion_reactions")
      return "показать реакции компаньона";
    if (tool === "mark_companion_reactions_seen")
      return "прочитать реакции компаньона";
    if (tool === "show_premium_capabilities")
      return "показать возможности Premium";
  ...
  private describeLimitTarget(input: Record<string, unknown>) {
    if (input.targetType === "account")
      return `лимит по счёту ${this.clean(input.account) || ""}`.trim();
    if (input.targetType === "category")
      return `лимит по категории ${this.clean(input.category) || ""}`.trim();
  ...
  private formatAmount(amount: unknown, currency: unknown) {
    ...
    const suffix =
      typeof currency === "string" && currency.trim() ? currency.trim() : "RUB";
