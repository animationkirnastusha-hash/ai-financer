...
Также backend пока принимает legacy intents. Если удобнее, можешь вернуть legacy JSON:
- batch
- expense
- income
- transfer
...
Но предпочтительный формат — toolCalls.
`;
...
function normalizeParsed(input: unknown): AIParsedCommand {
  const toolPlan = normalizeToolPlanToParsedCommand(input);
  ...
  if (intent === 'batch') {
    ...
    return {
      intent: 'batch',
      actions,
      originalText: asString(data.originalText || data.original || data.command, ''),
  ...
  if (intent === 'create_account') {
    ...
    return {
      ...
      type: ['cash', 'card', 'savings', 'investment'].includes(type) ? type : 'card',
